...
void QAmqpClientPrivate::initSocket() {
  ...
  QObject::connect(socket, SIGNAL(errorOccurred(QAbstractSocket::SocketError)), q,
                   SLOT(_q_socketError(QAbstractSocket::SocketError)));
  QObject::connect(socket, SIGNAL(errorOccurred(QAbstractSocket::SocketError)), q,
                   SIGNAL(socketError(QAbstractSocket::SocketError)));
  QObject::connect(socket, SIGNAL(stateChanged(QAbstractSocket::SocketState)), q,
                   SIGNAL(socketStateChanged(QAbstractSocket::SocketState)));
  QObject::connect(socket, SIGNAL(sslErrors(QList<QSslError>)), q,
                   SIGNAL(sslErrors(QList<QSslError>)));
}

void QAmqpClientPrivate::resetChannelState() {
  foreach (QString exchangeName, exchanges.channels()) {
    QAmqpExchange *exchange = qobject_cast<QAmqpExchange *>(exchanges.get(exchangeName));
    if (exchange) exchange->d_ptr->resetInternalState();
  }

  foreach (QString queueName, queues.channels()) {
    QAmqpQueue *queue = qobject_cast<QAmqpQueue *>(queues.get(queueName));
...
void QAmqpClientPrivate::parseConnectionString(const QString &uri) {
  ...
  if (connectionString.scheme() != AMQP_SCHEME && connectionString.scheme() != AMQP_SSL_SCHEME) {
  ...
  if (vhost.startsWith("/") && vhost.size() > 1) vhost = vhost.mid(1);
...
void QAmqpClientPrivate::_q_connect() {
  ...
  if (socket->state() != QAbstractSocket::UnconnectedState) {
    ...
    _q_disconnect();
    // We need to explicitly close connection here because either way it will not be closed until we
    // receive closeOk
    closeConnection();
  }
...
void QAmqpClientPrivate::_q_socketConnected() {
  if (reconnectTimer) reconnectTimer->stop();
  if (reconnectFixedTimeout == false) timeout = 0;
...
void QAmqpClientPrivate::_q_readyRead() {
  ...
  while (socket->bytesAvailable() >= QAmqpFrame::HEADER_SIZE) {
    ...
    if (socket->bytesAvailable() < readSize) return;
    ...
    switch (static_cast<QAmqpFrame::FrameType>(type)) {
      ...
      case QAmqpFrame::Body: {
        ...
        foreach (QAmqpContentBodyFrameHandler *methodHandler,
                 bodyHandlersByChannel[frame.channel()])
...
bool QAmqpClientPrivate::_q_method(const QAmqpMethodFrame &frame) {
  ...
  if (frame.methodClass() != QAmqpFrame::Connection) return false;

  if (closed) {
    if (frame.id() == QAmqpClientPrivate::miCloseOk) closeOk(frame);
    return false;
  }
...
void QAmqpClientPrivate::start(const QAmqpMethodFrame &frame) {
  ...
  qAmqpDebug(
      "-> connection#start( version_major=%d, version_minor=%d, mechanisms=(%s), locales=%s )",
...
void QAmqpClientPrivate::tune(const QAmqpMethodFrame &frame) {
  ...
  // b) if we configured them, use the smaller value, as otherwise the peer providing this value may
  // not work
  ...
  qAmqpDebug("-> connection#tune( channel_max=%d, frame_max=%d, heartbeat=%d )", channelMax,
             frameMax, heartbeatDelay);
...
void QAmqpClientPrivate::close(const QAmqpMethodFrame &frame) {
  ...
  qAmqpDebug("-> connection#close( reply-code=%d, reply-text=%s, class-id=%d, method-id:%d )", code,
             qPrintable(text), classId, methodId);
...
void QAmqpClientPrivate::secureOk() { qAmqpDebug("-> connection#secureOk()"); }
...
void QAmqpClientPrivate::tuneOk() {
  ...
  qAmqpDebug("<- connection#tuneOk( channelMax=%d, frameMax=%d, heartbeatDelay=%d )", channelMax,
             frameMax, heartbeatDelay);
...
QAmqpClient::QAmqpClient(QObject *parent) : QObject(parent), d_ptr(new QAmqpClientPrivate(this)) {
  Q_D(QAmqpClient);
  d->init();
}

QAmqpClient::QAmqpClient(QAmqpClientPrivate *dd, QObject *parent) : QObject(parent), d_ptr(dd) {}

QAmqpClient::~QAmqpClient() {
  Q_D(QAmqpClient);
  if (d->connected) d->_q_disconnect();
}
...
QAmqpExchange *QAmqpClient::createExchange(const QString &name, int channelNumber) {
  ...
  if (!name.isEmpty()) exchange->setName(name);
...
QAmqpQueue *QAmqpClient::createQueue(const QString &name, int channelNumber) {
  ...
  if (!name.isEmpty()) queue->setName(name);
  d->queues.put(queue);
  return queue;
}
...
int QAmqpClient::writeTimeout() const { return QAmqpFrame::writeTimeout(); }

void QAmqpClient::setWriteTimeout(int msecs) { QAmqpFrame::setWriteTimeout(msecs); }
...
QString QAmqpClient::gitVersion() { return QString(GIT_VERSION); }
